...
void AddRoundKey(uint8_t a[16], uint8_t roundkey[16]) {
	for (int i = 0; i <= 15; i++) {
		a[i] = a[i] ^ roundkey[i];
	}
}
...
uint8_t* encryption(uint8_t plaintext[16], uint8_t key[16]) {
	uint8_t* byte = transformation(plaintext);
	//Generate roundkey from original key.
...
uint8_t* decryption(uint8_t plaintext[16], uint8_t key[16]) {
	uint8_t* byte = transformation(plaintext);
	//Generate roundkey from original key.
	uint8_t** roundkey = (uint8_t * *)malloc(sizeof(uint8_t*) * 11);
	roundkey[0] = transformation(key);
...
int main() {
	//Enter the plaintext and key here.
	...
	printf("\n");
	uint8_t* plaintext_d = decryption(ciphertext, key);
	printf("The plaintext that is decrypted from the ciphertext is: \n");
	for (int i = 0; i <= 15; i++) {
		printf("%0x  ", plaintext_d[i]);
	}
	printf("\n");
	return 0;
}
